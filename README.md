...
- **빠른 입력**: 콤마로 구분한 후보를 한 번에 추가
  - `이름*3` 형태로 같은 후보를 여러 번 등록 (당첨 확률 가중치)
- **Google Sheets 불러오기**: 대량의 데이터를 Google Sheets로 관리
  - URL 입력만으로 간편하게 데이터 불러오기 (gviz CSV 엔드포인트로 프록시 없이 직접 fetch)
  - 자동으로 통합 데이터에 저장
...
### 🎨 미디어 자동 인식
- **이미지**: URL을 입력하면 자동으로 이미지 렌더링 (미리 로드해 결과 표시가 즉시 그려짐)
- **Google Drive**: 드라이브 공유 링크를 자동으로 표시 가능한 썸네일로 변환
...
- 🚫 **중복 방지**: 한 번 뽑힌 항목은 다시 뽑히지 않음 (남은 개수 표시)
  - "같은 이름이 뽑히면 모두 제거" 옵션으로 가중치 후보도 한 번만 당첨되게 설정 가능
  - "처음부터 다시" 버튼으로 초기화
